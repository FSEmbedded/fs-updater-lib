...
update_definitions::UBootBootstateFlags update_definitions::to_UBootBootstateFlags(unsigned char number)
{
    if (number == 0)
    {
...
update_definitions::UBootBootstateFlags update_definitions::to_UBootBootstateFlags(const std::string & number)
{
    if (number == "0")
    {
        return UBootBootstateFlags::NO_UPDATE_REBOOT_PENDING;
